...
void variableInit() {
	light_pos = glm::vec3(0.0, 0.0, 100.0);
...
GLvoid drawScene()
{
	...
	glUniform3fv(objColorLocation, 1, (float*)&objColor);
	modelTransform = glm::translate(glm::mat4(1.0f), glm::vec3(-20.0, 0, 0)) * glm::scale(glm::mat4(1.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	glUniformMatrix4fv(modelTransformLocation, 1, GL_FALSE, glm::value_ptr(modelTransform));
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);

	objColor = OBJECT_BLUE_COLOR;
	glUniform3fv(objColorLocation, 1, (float*)&objColor);
	modelTransform = glm::translate(glm::mat4(1.0f), glm::vec3(-30.0, 0, 0)) * glm::scale(glm::mat4(1.0f), glm::vec3(0.2f, 0.2f, 0.2f));
	glUniformMatrix4fv(modelTransformLocation, 1, GL_FALSE, glm::value_ptr(modelTransform));
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	case 'd':
		...
		break;
	case 'r':
		light_pos = glm::rotate(glm::mat4(1.0f), glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0)) * glm::vec4(light_pos, 1.0);
